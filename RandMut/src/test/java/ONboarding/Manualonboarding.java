 ...
 *  - Asks the user to type the path to the Excel file (instead of passing it as an argument)
 ...
 *  - Saves the changes back into the SAME file that was uploaded (in place)
 *
 ...
 *
 * The program will then ask you to type/paste the path to your Excel file.
 */
public class Manualonboarding {
    ...
    private static final int TEST_ID_AGE = 30;
    private static final int TEST_ID_MONTH = 1;
    private static final int TEST_ID_DAY = 1;
    ...
    public static void main(String[] args) {
        ...
        String filePath = askForFilePath(scanner);
        if (filePath == null) {
            return; // user gave up after repeated invalid paths
        }
        ...
        try (FileInputStream fis = new FileInputStream(filePath);
            ...
            // Save back into the SAME file that was uploaded (overwrite in place).
    ...
    /** Asks the user to type/paste the path to their Excel file, validating it exists and is an .xlsx file. */
    private static String askForFilePath(Scanner scanner) {
        final int maxAttempts = 5;
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            System.out.print("Please enter the full path to your Excel file (e.g. C:\\Users\\You\\Documents\\sheet.xlsx): ");
            String path = scanner.nextLine().trim();

            // Allow users who drag-and-drop a file in and end up with surrounding quotes.
            ...
            if (!file.exists()) {
                System.out.println("  Could not find a file at that path. Please check it and try again.");
                continue;
            }
            if (!path.toLowerCase().endsWith(".xlsx")) {
                System.out.println("  That file doesn't look like an .xlsx file. Please point to the Excel workbook.");
                continue;
            }
            return path;
        }
