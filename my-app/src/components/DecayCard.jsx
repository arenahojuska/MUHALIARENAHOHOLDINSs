...
const DecayCard = ({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Bound the movement
      const bound = 50;
      targetX = Math.max(Math.min(targetX, bound), -bound);
      targetY = Math.max(Math.min(targetY, bound), -bound);
      ...
      const cursorDistance = distance(cachedCursor.current.x, cursor.current.x, cachedCursor.current.y, cursor.current.y);
      imgValues.displacementScale = lerp(imgValues.displacementScale, map(cursorDistance, 0, 200, 0, 400), 0.06);
  ...
  return (
    <div
      ref={svgRef}
      className="relative rounded-2xl overflow-hidden shadow-xl cursor-pointer hover:shadow-2xl transition-shadow duration-500"
    ...
    >
      ...
      {/* Overlay Text */}
      <div className="absolute bottom-4 left-4 text-white font-extrabold text-2xl sm:text-3xl md:text-4xl drop-shadow-lg">
        {children}
      </div>
    </div>
  );
